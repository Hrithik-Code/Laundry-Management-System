...
export default function OrderDetails({ user }) {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:4000/customer/getUserOrder", {
          ...
          <TableBody>
            {order?.OrderDetails?.map((row, index) => (
              ...
              >
                ...
                <StyledTableCell>{row?.productId?.title}</StyledTableCell>
                <StyledTableCell>{row?.service_product}</StyledTableCell>
